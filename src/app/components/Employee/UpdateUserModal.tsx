...
export default function EmployeeFormData({
  ...
  }, [name, payroll, type, department, role, date]);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-sm">
      ...
      <div>
        ...
        {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
               ...
               <div>
          <label className="font-light text-black ml-1 mb-1 text-[17px]">Employee Email</label>
          <input
            type="email"
            name="email"
            ...
            className={`w-full rounded-[5px] border p-2 placeholder:text-[16px] ${errors.email ? "border-red-500" : ""}`}
          />
    ...
    {formData.employeeImage.length > 0 &&
      formData.employeeImage.map((file, idx) => (
        ...
        >
          ...
          <button
            ...
            className="absolute top-1 right-1 bg-red-500 text-white rounded-full text-sm w-5 h-5 flex items-center justify-center shadow hover:bg-red-600 transition"
          >
            ×
          </button>
        </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full border rounded p-2 mt-1 placeholder:text-[16px]"
        />
      </div>
